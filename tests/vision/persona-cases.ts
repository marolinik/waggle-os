...
const positiveFailureVerb = String.raw`(?<!not )(?<!cannot )(?<!can't )(?<!don't )(?<!doesn't )(?<!didn't )(?<!aren't )(?<!isn't )(?<!never )(?<!no longer )\b(?:currently\s+show(?:s|ing)?\s+(?:two|2)\s+remaining\s+failures?|(?:are|is)\s+(?:currently\s+|still\s+)?fail(?:ing)?\s+(?:in\s+)?(?:two|2)\s+(?:cases?|tests?)|(?:(?:still\s+)?(?:show(?:s|ing)?|have|report(?:s|ing)?|return(?:s|ing)?|produce(?:s|ing)?|retain(?:s|ed|ing)?)|remain(?:s|ing)?)\s+(?:two|2)\s+failures?)\b`;
