...
const positiveFailureVerb = String.raw`(?<!not )(?<!cannot )(?<!can't )(?<!don't )(?<!doesn't )(?<!didn't )(?<!aren't )(?<!isn't )(?<!never )(?<!no longer )\b(?:currently\s+show(?:s|ing)?\s+(?:two|2)\s+remaining\s+failures?|(?:are|is)\s+(?:currently\s+|still\s+)?fail(?:ing)?\s+(?:in\s+)?(?:two|2)\s+(?:cases?|tests?)|(?:(?:still\s+)?(?:show(?:s|ing)?|have|report(?:s|ing)?|return(?:s|ing)?|produce(?:s|ing)?|retain(?:s|ed|ing)?|reveal(?:s|ing)?)|remain(?:s|ing)?)\s+(?:two|2)\s+(?:(?:outstanding|persistent|remaining)\s+)?failures?)\b`;
...
const directFundingAction = String.raw`(?:secure|obtain|arrange)\b[ \t]+(?:(?:an?|additional|new|short[- ]term|near[- ]term|emergency|external|temporary|working[- ]capital)\b[ \t]+){0,3}(?:bridge[ \t]+loan|loan|financing|funding|(?:line|facility)[ \t]+of[ \t]+credit)(?=[ \t]*(?:$|[.,;:!?|]|\r?\n|\bor[ \t]+pre[- ]?sell\b|\bto[ \t]+(?:boost|extend|improve|increase|provide|raise|secure)\b|\bfor[ \t]+(?:cash[ -]?flow|operations?|payroll|working[ -]?capital)\b|\bto[ \t]+cover[ \t]+(?:cash[ -]?flow|operating[ \t]+costs?|operations?|payroll|working[ -]?capital)\b|\bfrom[ \t]+(?:(?:an?|our|the)[ \t]+)?(?:bank|lender)\b))(?![^.\r\n]{0,100}\b(?:(?:approval|application|disbursement|financing|funding|it|loan)[ \t]+(?:(?:was|is)[ \t]+|has[ \t]+been[ \t]+)(?:denied|cancelled|canceled|rejected)|(?:no|zero|none)\s+(?:cash|funds?|funding|proceeds?)|without\b[^.\r\n]{0,30}(?:cash|funds?|funding|proceeds?)|(?:has|have|had)[ \t]+no[ \t]+effect\b|(?:not|never|no longer|(?:won|wouldn|doesn|didn|isn|aren)['’]t|fails?[ \t]+to)\b[^.\r\n]{0,30}(?:improve|increase|extend|provide|receive|raise|secure)\b))`;
