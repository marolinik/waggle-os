...
const runwayFormulaPattern = new RegExp([
  `${runwayAssertionPrefix}${String.raw`(?<![\d,.])\$?\s*40[,.]?000(?:\.0{1,2})?(?![\d,]|\.\d)\s*(?:/|÷|divided by)\s*(?<![\d,.])\$?\s*10[,.]?000(?:\.0{1,2})?(?![\d,]|\.\d)`}${runwayAssertionSuffix}`,
  `${runwayAssertionPrefix}${String.raw`cash(?:\s+(?:balance|on\s+hand))?\s*(?:/|÷|divided by)\s*(?:(?:net\s+)?monthly\s+burn|monthly\s+net\s+burn|net\s+burn|burn)`}${runwayAssertionSuffix}`,
  `${runwayAssertionPrefix}${String.raw`\\frac\s*\{\s*\\text\s*\{\s*cash(?:\s+(?:balance|on\s+hand))?\s*\}\s*\}\s*\{\s*\\text\s*\{\s*(?:(?:net\s+)?monthly\s+burn|monthly\s+net\s+burn)\s*\}\s*\}`}${runwayAssertionSuffix}`,
