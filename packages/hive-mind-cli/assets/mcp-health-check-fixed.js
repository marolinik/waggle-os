...
/**
 * BUNDLED ASSET — mcp-health-check.js for hive-mind-cli postinstall override.
 *
 * Source: everything-claude-code marketplace plugin
 *   (https://github.com/Affaan-Mustafa/everything-claude-code)
 *   commit cf6e6c5d, MIT License (Copyright (c) 2026 Affaan Mustafa)
 *   Patch authored by Marko Markovic <marko.markovic@egzakta.com> at Egzakta Group.
 *
 * Patch summary:
 *   probeCommandServer() now sets `shell: true` and `windowsHide: true` on the
 *   spawnOptions when process.platform === 'win32'. This lets npm-installed
 *   .cmd / .bat shims (e.g., hive-mind-cli) resolve via cmd.exe instead of
 *   failing with `spawn ENOENT` and getting quarantined for the entire backoff
 *   window.
 *
 * Distribution: Apache-2.0 wrapper around MIT-licensed upstream. The MIT notice
 * above is preserved per the original LICENSE in everything-claude-code repo.
 * MIT and Apache-2.0 are bidirectionally compatible for redistribution.
 *
 * Lifecycle: this asset is bundled by @waggle/hive-mind-cli and dropped to
 * ~/.claude/scripts/hooks/mcp-health-check.js (user override path) on win32 by
 * the package's postinstall script. Once upstream merges the fix (PR materials
 * in packages/hive-mind-hooks-claude-code/upstream-pr/), this override is no
