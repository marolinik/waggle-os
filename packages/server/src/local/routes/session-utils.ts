/**
 * Session utility functions — pure logic with zero Fastify/server dependency.
 *
 * Extracted from sessions.ts to keep route handlers thin.
 * All functions operate on file paths and directories; none require server state.
 */
...
export type ThreadFreshness = 'fresh' | 'aging' | 'stale';
...
function generateSessionSummary(messageLines: string[]): string | null {
  ...
  const decisionPatterns = [
    ...
    /\b(?:let'?s (?:proceed|move forward|do that))\b/i,
  ];
 ...
 */
export function readSessionMeta(filePath: string, sessionId: string): SessionInfo {
 ...
 */
export const TOOL_CONTENT_PATTERNS: Array<{ pattern: RegExp; tool: string }> = [
