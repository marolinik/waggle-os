...
- [ ] Founder ratification list from the Phase 4 panel: **F1** catch-convention amendment (fail closed *and* warn), **F2** trust-half security posture, **F3** governance end state (discriminated return), **F5** keep both resolvers for Phase 4, **F8** `/memory` recall cap is exempt from the constants rule, **F12** `ChatTurnScope` before the method object, **F14** no proof-of-equivalence carve-out in Rule 8, **F15** pins assert HTTP/SSE/persisted observables only (founder)
