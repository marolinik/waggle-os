...
- [ ] **Founder decision**: should a failed `install_capability` trust assessment deny the install, the way an unclassifiable tool now does, instead of silently offering the weaker `medium`/`elevated` class? (TD-CHAT-38 remainder)
- [ ] **Founder decision**: does the TD-CHAT-36 fix accept the deny-to-prompt posture change a total `describeToolUse` implies at the hook sites, which `cc3e1436` declined? If not, the fix must deny explicitly rather than coerce safely
