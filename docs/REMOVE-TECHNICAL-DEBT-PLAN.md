...
| 2026-09-15 | 1/3 | `buildChatCommandContext` pinned directly — a one-token export (`80006984`) plus 14 unit pins with a stub orchestrator and stub server (`9a9f96b7`) — rather than through the route; a recall-outage quirk found while pinning was pinned and ledgered (TD-CHAT-29), not fixed | Rule 8: found bugs get pinned, not fixed; direct pins were the cheapest test point once the helper was module-level, and the route cannot reach the throwing-`recallMemory` branch at all. |
