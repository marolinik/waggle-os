...
- `adapter.fetch()` throws `AdapterHttpError` on `!res.ok` after the 403 dispatch (dispatch-then-throw). Error shape: `class AdapterHttpError extends Error { status: number; body: unknown; code?: string }` — **compatible with the existing rich-error consumers**: `CapabilitiesApp.handleInstallError` reads `err.status === 403` / `e.body?.required` (`CapabilitiesApp.tsx:191-204`), SkillBuilder `onTierError` (`SkillBuilder.tsx:141`); the throwing convention is already the incumbent for `installSkill`/`installPack` (`adapter.ts:914-920/960-966`). Body parsed best-effort via `res.clone().json()`. **Message pinned (verified — 6+ surfaces render `err.message`):** `AdapterHttpError.message := String(body.error ?? body.message ?? \`HTTP ${status}\`)`, test-pinned, so server reasons ("steps is mandatory", 413 backup-too-large, spawn timeouts) survive the conversion.
- New method `adapter.fetchRaw()` = today's semantics (never throws on status; still attaches token, still 403-dispatches, still deferral+retry). **Two migration classes** (plan-verify 2026-06-11 corrected the original filter — "status-aware handling on the Response" misses *body-envelope* consumers):
  - **Raw-Response callers:** `installMarketplacePackage` (callers check `res.status === 403` + SecurityGate `{blocked,severity}` envelope: `MarketplaceApp.tsx:228-245`, `CapabilityRequestCard.tsx:54-62`) — documented raw contract `adapter.ts:981-995`. **NOT `uninstallMarketplacePackage`** (verified: its only consumer ignores the Response and unconditionally toasts success, `MarketplaceApp.tsx:253-262` — leaving it on throwing fetch fixes a silent false-success bug for free; test-pin that a 500 uninstall shows the failure toast and does NOT flip `installed:false`).
  - **Body-envelope getters (CRITICAL finding):** typed getters whose *parsed error body* is load-bearing to consumers. Verified set: `installMcp` (`adapter.ts:1663-1671` — server sends `{installed:false, requiresApproval, blocked, severity, scanResult}` on 403/422 per `mcps.ts:249-259`; `MCPHubApp.tsx:156-182` branches `res.error === 'TIER_INSUFFICIENT'` AND drives the ApprovalModal HIGH-override / CRITICAL-non-overridable flow off `res.requiresApproval` — throw-on-`!ok` would kill that flow), plus `addCustomMcp` (`AddCustomMcpForm.tsx:53-68`, deliberate tier-403 inline-suppression), `updateMcpPermissions` (`MCPHubApp.tsx:240-247` — comment literally pins the never-throws contract), `revokeMcp` (`MCPHubApp.tsx:216-226`), `startMcp`/`stopMcp`/`testMcp` (`InstalledMcpList.tsx:54-78`). Migration: switch internals to `fetchRaw`, keep returning parsed body regardless of status. **Pin each at the ADAPTER level** (real method + mocked fetch returning 422/403 envelopes) — the existing phase4b component tests mock the adapter methods themselves and are structurally blind to this regression class.
  - Implementation sweep to close the class: grep components for `res.error` / `res.ok`-on-non-Response / `requiresApproval` / `blocked` / `scanResult` before finalizing the migration list.
  - `createSkill` (`adapter.ts:866-875`) already throws a rich error whose message = `body.error` — with the message pin above, `AdapterHttpError` is a verified drop-in; simplify onto throwing `fetch` and test-pin message+status+body.
  - The 15 direct `adapter.fetch(` component call sites — verified per-site: **only BackupApp needs `fetchRaw`** (`BackupApp.tsx:41` maps 404→'empty' state via `classifyMetadataStatus:21-25` — on throwing fetch the legitimate no-backups 404 becomes a permanent error panel; `:59`/`:89` branch `res.ok` with distinct server-fault messages incl. the 413 reason). The other 12 (TelemetryApp×2, SkillEditorDrawer, SkillBuilder, EvolutionTab×8) stay on throwing `fetch` — their `!ok→throw` branches become dead code and catch paths *improve*. Add a BackupApp component-level 404→empty test (the existing `phase5b-backup.test.tsx` tests only the pure function — vacuous against a missed migration). Rule for any stragglers: migrate only where a non-2xx status maps to a NON-error UI state or where status/body drive distinct UI handling a catch cannot reproduce.
...
- `sendMessage` (chat SSE generator, `adapter.ts:380-398`): today a 4xx/5xx parses the error body as an SSE stream and yields nothing. With throwing `fetch` the generator now throws. **Verified consumer is `useChat.ts:125`** (ChatWindowInstance only wires the hook): the assistant placeholder at `:114` precedes the for-await, so the throw lands in the catch at `:264` and renders — no crash — BUT that catch (i) hardcodes "Backend is offline" (would misreport every 401/403/500 post-change) and (ii) lacks the `!last` empty-array guard the stream updater has (`:133-136`). Fix both in Stage B: branch on `AdapterHttpError` (status/body-derived message; suppress the inline message for tier-403 the UpgradeModal already handles), keep "Backend is offline" for genuine network errors only, add the `!last` guard. Test targets `useChat` with a rejecting `adapter.sendMessage`.
...
- `CapabilityRequestCard.tsx:55` treats ANY 403 as a tier rejection (no `TIER_INSUFFICIENT` check) — pre-existing, not P1b-induced; ledger for P7 (a SecurityGate 403 there would misrender as "Upgrade required").
...
5. **403/UpgradeModal surface intact:** existing `phase4b-mcp-hub` / `phase4b-marketplace-extend` / `phase3c-skill-builder` regression tests stay green; 403 dispatch fires on both throwing and raw paths. **Plus adapter-level envelope pins** (component tests mock adapter methods and are blind to the fetch-layer change): real `installMcp` against mocked 422 `requiresApproval` + 403 `TIER_INSUFFICIENT` responses returns the parsed envelope, not a throw; same for the other body-envelope getters.
