...
scores \best{87.66\%} overall --- \best{+5.71pp} above the best
previously published memory system (Memori, 81.95\%; one-sample
$z{=}4.42$, $p{<}10^{-5}$) --- and leads or ties every question
...
field. The substrate, including the benchmark harness, is open source.
Token cost (3{,}747 tokens/question vs.\ Memori's 1{,}294) remains the
one axis where prior work dominates; we discuss the planned packing
work.
\end{abstract}
...
\item \textbf{A SOTA result under a reproduced protocol.} We score
  \best{87.66\%} overall on LoCoMo under Memori's own published
  protocol and judge, $+5.71$pp over Memori's 81.95\% ($z{=}4.42$,
...
\midrule
\textbf{\system{} (ours, W3.3)} & \best{92.75} & \best{82.98} & \best{83.49} & \best{70.83} & \best{87.66}\\
Memori \cite{memori} & 87.87 & 72.70 & 80.37 & 63.54 & 81.95\\
Zep$^\dagger$ \cite{zep} & 83.49 & 72.34 & 77.26 & 64.58 & ---\\
LangMem$^\dagger$ & 86.92 & 74.47 & 61.06 & 67.71 & 78.05\\
Mem0 \cite{mem0} & 66.47 & 62.41 & 57.32 & 44.79 & 62.47\\
\midrule
...
\smallskip
\raggedright\footnotesize $^\dagger$Cross-sourced from Du et
...
\textbf{87.66 vs.\ 81.95, $+5.71$pp, $z{=}4.42$, $p{<}10^{-5}$}. Our
...
\item \textbf{Token cost.} 3{,}747 tokens/question vs.\ Memori's
  ...
  raw-detail excerpts; per-lane token budgeting is the planned next
  step, targeting $\le$1{,}500/q at iso-accuracy.
...
\item \textbf{Cross-sourced baselines.} Zep/LangMem/Mem0 rows are not
  same-harness; only the Memori comparison carries the statistical
  claim. Re-running the major baselines in our harness is planned.
...
escalation lane for fine-grained recall --- achieves 87.66\% on LoCoMo
...
open source at \url{https://github.com/marolinik/hive-mind}.
