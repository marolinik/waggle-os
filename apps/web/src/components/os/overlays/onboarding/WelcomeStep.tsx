...
const WelcomeStep = ({ onClickAnywhere, offline }: WelcomeStepProps) => {
  ...
  >
    ...
    <button
      ...
      autoFocus
      className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-primary text-primary-foreground font-display text-sm font-semibold hover:bg-primary/80 transition-colors focus-visible:outline-none focus-visible:shadow-[0_0_0_3px_var(--focus-ring)]"
    >
      Continue →
    </button>

    {/* Local-first / privacy note (C28 companion). Lane T: was
        text-muted-foreground/70 — the /70 composited to ~2.9:1 (light) / 3.8:1
        (dark) over the scrim (sub-AA). Full-opacity --text-tertiary → 5.4/7.0:1. */}
    <p className="flex items-center justify-center gap-1.5 text-xs text-[var(--text-tertiary)] mt-5 max-w-md mx-auto">
    ...
    {offline && (
      <p className="flex items-center justify-center gap-1.5 text-[11px] text-[var(--text-tertiary)] mt-2">
