...
const VaultApp = () => {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-full"><Loader2 className="w-5 h-5 text-honey animate-spin" /></div>;
  }
      ...
      <div className="w-36 border-r border-border/50 p-2 space-y-0.5 shrink-0" role="tablist" aria-label="Vault sections">
        <button onClick={() => setTab('secrets')}
          ...
          className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs transition-colors ${
            tab === 'secrets' ? 'bg-primary/20 text-honey' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
          }`}>
        ...
        {tab === 'secrets' && (
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-2.5 p-3 rounded-lg bg-primary/10 border border-primary/30">
              <Shield className="w-4 h-4 mt-0.5 text-honey shrink-0" />
              <div className="text-xs text-foreground leading-relaxed space-y-1">
                <p>
                  <strong className="text-honey">The Vault is the single source for API keys.</strong>
                  {' '}Values are encrypted with <strong>AES-256-GCM</strong> and stored locally at{' '}
                  <code className="px-1 py-0.5 rounded bg-muted text-[10px] text-honey">~/.waggle/vault</code>.
                </p>
