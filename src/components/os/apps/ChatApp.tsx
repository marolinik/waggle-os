...
import { Send, Sparkles, Plus, Slash, Paperclip, ChevronDown, ThumbsUp, ThumbsDown, Loader2, AlertTriangle, CheckCircle2, XCircle, Clock } from 'lucide-react';
...
const ToolCard = ({ tool }: { tool: ToolExecution }) => (
  <div className="my-1 rounded-lg border border-border/50 bg-secondary/50 p-2">
    ...
    {tool.output && (
      <pre className="mt-1 text-[10px] text-muted-foreground bg-background/50 rounded p-1.5 overflow-x-auto max-h-24">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <div ref={scrollRef} className="flex-1 overflow-auto p-4 space-y-3">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-xs text-muted-foreground">Type a message or use / for commands</p>
            </div>
          )}
          {messages.map((msg) => (
        ...
        <div className="p-3 border-t border-border/30 relative">
          {showSlash && filteredCommands.length > 0 && (
            <div className="absolute bottom-full left-3 right-3 mb-1 bg-card border border-border rounded-xl shadow-xl overflow-hidden z-10">
