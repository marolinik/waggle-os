...
- ✅ C-1 LoCoMo Memory SOTA — **CURRENT CANONICAL: 87.66% overall** (W3.3, Memori *same-judge* protocol — GPT-4.1-mini answerer+judge, N=1540), **+5.71pp over Memori 81.95** (z=4.42, p<10⁻⁵), leading/tying every category. Mem0 re-run on our ruler = **73.96** overall (**temporal +32.7pp** landslide; write-time dating vs Mem0 ingestion-time). **W4 production port COMPLETE**; paper + arXiv draft + team briefing in [`docs/paper/`](docs/paper/). Evidence index: `memory/project_sota_results_index.md` (hive-mind-test master @ `05f2146`; OSS `marolinik/hive-mind` @ `bc4eba1`, PR #14).
